...
describe('MigrationErrorDialog', () => {
  ...
  it('renders the Database Error heading', () => {
    render(<MigrationErrorDialog errorMessage="test error" onQuit={vi.fn()} />);
    expect(screen.getByText('Database Error')).toBeInTheDocument();
  });

  it('renders the provided error message', () => {
    render(<MigrationErrorDialog errorMessage="WAL mode failed" onQuit={vi.fn()} />);
    expect(screen.getByText('WAL mode failed')).toBeInTheDocument();
  });

  it('calls onQuit when Quit App button is clicked', () => {
    const onQuit = vi.fn();
    render(<MigrationErrorDialog errorMessage="err" onQuit={onQuit} />);
    fireEvent.click(screen.getByRole('button', { name: /quit app/i }));
    expect(onQuit).toHaveBeenCalledOnce();
  });

  it('does NOT dismiss on Escape keypress', () => {
    render(<MigrationErrorDialog errorMessage="err" onQuit={vi.fn()} />);
