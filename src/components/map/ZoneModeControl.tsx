...
export function ZoneModeControl({
  ...
  const OPTIONS: Array<{ mode: ZoneViewMode; label: string }> = [
    ...
    { mode: 'region', label: t('zone.regionMode') },
    { mode: 'all', label: t('zone.allZones') },
  ];
  ...
  return (
    ...
    >
      ...
      {collapsed ? (
        <button
          type="button"
          aria-label={t('zone.showControls')}
      ...
      <div className="flex w-fit max-w-full items-center gap-1 rounded-md border border-primary/35 bg-card/95 p-0.5 shadow-xl backdrop-blur">
        ...
        <button
          type="button"
          aria-label={t('zone.hideControls')}
            ...
            <>
              <div className="mt-2 flex flex-col gap-1.5">
                ...
                >
                  ...
                  {hasRegionPolygon ? t('zone.editRegionPolygon') : t('zone.drawRegionPolygon')}
                </button>
                <button
                  type="button"
                ...
                >
                  <PencilRuler className="h-3.5 w-3.5" />
                  {creatingRegion ? t('zone.opening') : hasRegionZone ? t('zone.editRegionCircle') : t('zone.addQuickCircle')}
                </button>
              </div>
        ...
        <div className="w-fit max-w-[250px] rounded-md border border-border bg-card/95 p-2 text-xs text-muted-foreground shadow-xl backdrop-blur">
          ...
          {localTargetFind ? (
            <div className="mt-2 flex flex-col gap-1.5">
              ...
              >
                <PencilRuler className="h-3.5 w-3.5" />
                {hasLocalPolygon ? t('zone.editLocalPolygon') : t('zone.drawLocalPolygon')}
              </button>
              <button
                type="button"
              ...
              >
                <PencilRuler className="h-3.5 w-3.5" />
                {hasLocalCircle ? t('zone.editLocalCircle') : t('zone.addQuickCircle')}
              </button>
            </div>
          ) : (
            <div className="mt-2 rounded-md border border-border/60 bg-secondary/10 px-2 py-2 text-[10px] leading-relaxed text-muted-foreground/85">
              {t('zone.pickFindFirst')}
            </div>
          )}
        </div>
      )}
